...
#define I2S_Format        3 // 0=16bit, 2=24bit, 3=32bit
//3 = I2S, 32 bits
//19 = TDM/DSP, 32 bits
//35 = RTJ, 32 bits
...
void setup() {
  ...
  //  delay(5000);
  //  Serial.println("AT+I2SCFG=65"); //Set I2S/PCM configuration to: I2S master, 32-bit resolution,48kHz. https://document.feasycom.com/web/#/2.1.22
  ////  Serial.println("AT+I2SCFG=33"); //Set I2S/PCM configuration to: I2S master, 24-bit resolution,48kHz. https://document.feasycom.com/web/#/2.1.22
  ////  Serial.println("AT+I2SCFG=49"); //Set I2S/PCM configuration to: I2S master, 24-bit resolution,44kHz. https://document.feasycom.com/web/#/2.1.22
  //    delay(5000);
  //
  //  Serial.println("AT+REBOOT"); //Soft reboot
  //    delay(5000);
  ...
  //  Serial.println("AT+I2SCFG=65"); //Set I2S/PCM configuration to: I2S master, 32-bit resolution,48kHz. https://document.feasycom.com/web/#/2.1.22
  //    delay(5000);
  //  Serial.println("AT+I2SCFG"); //Set I2S/PCM configuration to: I2S master, 32-bit resolution,48kHz. https://document.feasycom.com/web/#/2.1.22
  //    delay(5000);
  //  Serial.println("AT+I2SCFG"); //Set I2S/PCM configuration to: I2S master, 32-bit resolution,48kHz. https://document.feasycom.com/web/#/2.1.22
  //
  //
  //  digitalWrite( 5, HIGH );//BT-806 Vreg turn on
  //  delay(2000);
  //  digitalWrite( 5, LOW );//BT-806 Vreg turn off
  //
...
void loop() {
  ...
  if ( currentMillis - previousMillis >= 1000 ) {
    ...
    /////////  DEVICE_CTRL_2 (2)
    Serial.print("**DEVICE_CTRL_2 (2) 0x03: \t");
    Serial.print( TAS5825.getDEVICE_CTRL_22(), HEX );
    if ( TAS5825.getDEVICE_CTRL_22() == 16 ) {
      Serial.println(",HEX >>>Reset the DSP");
    }
    else if ( TAS5825.getDEVICE_CTRL_22() == 3 )
    {
      Serial.println(",HEX >>>Play");
    }
    else if  ( TAS5825.getDEVICE_CTRL_22() == 0 )
    {
      Serial.println(",HEX >>>MUTE ");
    }
    ...
    /////////  CLKDET_STATUS (2)
    // TO DO; see what is up with this REG
    Serial.print("--CLKDET_STATUS (2)  0x39 : \t");
    Serial.print( TAS5825.getCLKDET_STATUS2(), HEX );
    if ( TAS5825.getCLKDET_STATUS2() == 8 ) {
      Serial.println(",HEX >>>Normal Opertion");
    }
    else {
      Serial.println(",HEX >>>Look At the Datasheet it should be 3,HEX");
    ...
    /////////  POWER_STATE (2)
    Serial.print("--POWER_STATE (2)  0x68 : \t");
    Serial.print( TAS5825.getPOWER_STATE2(), HEX );
    if ( TAS5825.getPOWER_STATE2() == 3 ) {
      Serial.println(",HEX >>>Play");
    }
    else if ( TAS5825.getPOWER_STATE2() == 2) {
      Serial.println(",HEX >>>HIZ");
    }
    else if ( TAS5825.getPOWER_STATE2() == 1 ) {
      Serial.println(",HEX >>>sleep");
    }
    else {
      Serial.println(",HEX >>>Deep Sleep");
    ...
    /////////  CHAN_FAULT2
    Serial.print("--CHAN_FAULT (2) 0x70 : \t");
    Serial.print( TAS5825.getCHAN_FAULT2(), HEX );
    if ( TAS5825.getCHAN_FAULT2() == 0 ) {
      Serial.println(",HEX >>>Normal Opertion");
    }
    else {
      Serial.println(",HEX >>>Look At the Datasheet it should be 0,HEX");
    ...
    /////////  AUTOMUTE_STATE
    Serial.print("--AUTOMUTE_STATE  0x69 : \t");
    ...
    /////////  GLOBAL_FAULT11
    Serial.print("--GLOBAL_FAULT1 (2)  0x71 : \t");
    Serial.print( TAS5825.getGLOBAL_FAULT11(), HEX );
    if ( TAS5825.getGLOBAL_FAULT11() == 0 ) {
      Serial.println(",HEX >>>Normal Opertion");
    }
    else if ( TAS5825.getGLOBAL_FAULT1() == 4 ) {
      Serial.println(",HEX >>>Clock fault");
    ...
    /////////  GLOBAL_FAULT22
    Serial.print("--GLOBAL_FAULT2 (2)  0x72 : \t");
    Serial.print( TAS5825.getGLOBAL_FAULT22(), HEX );
    if ( TAS5825.getGLOBAL_FAULT22() == 0 ) {
      Serial.println(",HEX >>>Normal Opertion");
    }
    else {
      Serial.println(",HEX >>>Look At the Datasheet it should be 0,HEX");
    ...
    Serial.print( TAS5825.getWARNING(), BIN );
    ...
    Serial.print("FS_MON (2): \t\t");
    Serial.print( TAS5825.getFS_MON2(), DEC );
    if ( TAS5825.getFS_MON2() == 9 ) {
      Serial.println(",DEC >>> Sampling Rate = 48KHz");
    }
    else if ( TAS5825.getFS_MON2() == 11 ) {
      Serial.println(",DEC >>>> Sampling Rate = 96KHz");
    }
    else if ( TAS5825.getFS_MON2() == 0 ) {
      Serial.println(",DEC >>>> FS Error");
    }
